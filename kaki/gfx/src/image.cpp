...
void kaki::imageLoadHandler(flecs::iter iter, kaki::Asset *assets) {
    ...
    for(auto i : iter) {
        ...
        ktxTexture* kTexture;
        KTX_error_code result = ktxTexture_CreateFromNamedFile(asset.path, KTX_TEXTURE_CREATE_NO_FLAGS, &kTexture);
        if (result != KTX_SUCCESS) {
            fprintf(stderr, "Failed to load image %s", asset.path);
            return;
        }
        ...
        if (result != KTX_SUCCESS) {
            fprintf(stderr, "Failed to upload texture %s", asset.path);
        }
